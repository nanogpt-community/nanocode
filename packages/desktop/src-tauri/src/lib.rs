...
const SETTINGS_STORE: &str = "nanocode.settings.dat";
...
fn spawn_sidecar(app: &AppHandle, hostname: &str, port: u32, password: &str) -> CommandChild {
    ...
    .env("NANOGPT_SERVER_USERNAME", "nanocode")
    .env("NANOGPT_SERVER_PASSWORD", password)
    .spawn()
    .expect("Failed to spawn nanocode");
...
async fn check_server_health(url: &str, password: Option<&str>) -> bool {
    ...
    if let Some(password) = password {
        req = req.basic_auth("nanocode", Some(password));
    }
...
async fn spawn_local_server(
    ...
    loop {
        if timestamp.elapsed() > Duration::from_secs(30) {
            break Err(format!(
                "Failed to spawn NanoCode Server. Logs:\n{}",
