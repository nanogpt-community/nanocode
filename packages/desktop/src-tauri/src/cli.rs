...
fn get_user_shell() -> String {
    std::env::var("SHELL").unwrap_or_else(|_| "@nanogpt/bin/sh".to_string())
}
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        ...
        let line = if shell.ends_with("@nanogpt/nu") {
...
mod tests {
    ...
    fn parse_shell_env_supports_null_delimited_pairs() {
        ...
        assert_eq!(env.get("PATH"), Some(&"@nanogpt/usr/bin:/bin".to_string()));
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        shell_env.insert("PATH".to_string(), "@nanogpt/shell/path".to_string());
        shell_env.insert("HOME".to_string(), "@nanogpt/tmp/home".to_string());

        let merged = merge_shell_env(
            Some(shell_env),
            vec![
                ("PATH".to_string(), "@nanogpt/desktop/path".to_string()),
                ("NANOGPT_CLIENT".to_string(), "desktop".to_string()),
            ],
        )
        .into_iter()
        ...
        assert_eq!(merged.get("PATH"), Some(&"@nanogpt/desktop/path".to_string()));
        assert_eq!(merged.get("HOME"), Some(&"@nanogpt/tmp/home".to_string()));
        assert_eq!(merged.get("NANOGPT_CLIENT"), Some(&"desktop".to_string()));
    }

    #[test]
    fn is_nushell_handles_path_and_binary_name() {
        assert!(is_nushell("nu"));
        assert!(is_nushell("@nanogpt/opt/homebrew/bin/nu"));
        assert!(is_nushell("C:\\Program Files\\nu.exe"));
        assert!(!is_nushell("@nanogpt/bin/zsh"));
    }
}
