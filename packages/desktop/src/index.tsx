...
import { Splash } from "@nanogpt/ui/logo"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__NANOGPT__.deepLinks = [...pending, ...urls]
...
const createPlatform = (password: Accessor<string | null>): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__NANOGPT__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__NANOGPT__?.wsl) return result
  ...
  return {
    ...
    getWslEnabled: async () => {
      ...
      return window.__NANOGPT__!.wsl ?? false
    },
