...
import { Avatar } from "@nanogpt/ui/avatar"
...
import { Session, type Message, type TextPart } from "@nanogpt/sdk/v2/client"
...
const NANOCODE_PROJECT_ID = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"
...
export default function Layout(props: ParentProps) {
  ...
  const deepLinkEvent = "nanocode:deep-link"

  const parseDeepLink = (input: string) => {
    if (!input.startsWith("nanocode://")) return
  ...
  const drainDeepLinks = () => {
    const pending = window.__NANOGPT__?.deepLinks ?? []
    if (pending.length === 0) return
    if (window.__NANOGPT__) window.__NANOGPT__.deepLinks = []
    ...
    return (
      <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
        <div class="size-full rounded overflow-clip">
          <Avatar
            fallback={name()}
            src={props.project.id === NANOCODE_PROJECT_ID ? "https://nanocode.ai/favicon.svg" : props.project.icon?.override}
