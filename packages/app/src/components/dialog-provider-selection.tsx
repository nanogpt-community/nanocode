...
export const DialogProviderSelection = (props: {
    modelId: string
}) => {
    const sdk = useSDK()
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const modelRes = await fetch(`${sdk.url}/api/models/${encodeURIComponent(props.modelId)}/providers`)
                                ...
                                return (
                                    <div class="flex items-center justify-between p-2 rounded-md hover:bg-surface-raised-base">
                                        <div class="flex items-center gap-2">
                                            ...
                                            />
                                            <span class={!p.available ? "opacity-50 line-through text-13-medium" : "text-13-medium"}>{p.provider}</span>
                                        ...
                                        <Show when={isSelected()}>
                                            <div class="flex gap-1">
                                                ...
                                                <Button size="small" variant="ghost" onClick={() => moveDown(index())} disabled={index() === preferredList().length - 1}>
