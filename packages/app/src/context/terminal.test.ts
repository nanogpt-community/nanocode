...
beforeAll(async () => {
  ...
  mock.module("@nanogpt/ui/context", () => ({
    createSimpleContext: () => ({
      use: () => undefined,
      provider: () => undefined,
    }),
...
describe("getWorkspaceTerminalCacheKey", () => {
  test("uses workspace-only directory cache key", () => {
    expect(getWorkspaceTerminalCacheKey("@nanogpt/repo")).toBe("@nanogpt/repo:__workspace__")
  })
})

describe("getLegacyTerminalStorageKeys", () => {
  test("keeps workspace storage path when no legacy session id", () => {
    expect(getLegacyTerminalStorageKeys("@nanogpt/repo")).toEqual(["@nanogpt/repo/terminal.v1"])
  })

  test("includes legacy session path before workspace path", () => {
    expect(getLegacyTerminalStorageKeys("@nanogpt/repo", "session-123")).toEqual([
      "@nanogpt/repo/terminal/session-123.v1",
      "@nanogpt/repo/terminal.v1",
    ])
  })
})
