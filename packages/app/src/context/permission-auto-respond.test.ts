...
import type { PermissionRequest, Session } from "@nanogpt/sdk/v2/client"
...
describe("autoRespondsPermission", () => {
  ...
  test("uses a parent session's legacy auto-accept key", () => {
    ...
    expect(autoRespondsPermission({ root: true }, sessions, permission("child"), "@nanogpt/tmp/project")).toBe(true)
  })
  ...
  test("defaults to requiring approval when no lineage override exists", () => {
    ...
    expect(autoRespondsPermission(autoAccept, sessions, permission("child"), "@nanogpt/tmp/project")).toBe(false)
  })
...
describe("isDirectoryAutoAccepting", () => {
  ...
  test("returns false when directory key is not set", () => {
    expect(isDirectoryAutoAccepting({}, "@nanogpt/tmp/project")).toBe(false)
  })
