...
:::note
Ovaj alat je dostupan samo kada je `NANOGPT_EXPERIMENTAL_LSP_TOOL=true` (ili `NANOGPT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `NANOGPT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
