...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti otvorenog koda bez TUI interfejsa. Postavite `NANOGPT_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenCode preko web interfejsa. Postavite `NANOGPT_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
| `NANOGPT_GIT_BASH_PATH` | string | Putanja do Git Bash izvršne datoteke na Windows |
| `NANOGPT_CONFIG` | string | Put do konfiguracionog fajla |
| `NANOGPT_CONFIG_DIR` | string | Put do konfiguracijskog direktorija |
...
| `NANOGPT_DISABLE_PRUNE` | boolean | Onemogući orezivanje starih podataka |
| `NANOGPT_DISABLE_TERMINAL_TITLE` | boolean | Onemogući automatsko ažuriranje naslova terminala |
| `NANOGPT_PERMISSION` | string | Umetnuta json konfiguracija dozvola |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS` | boolean | Onemogući podrazumevane dodatke |
...
| `NANOGPT_DISABLE_CLAUDE_CODE` | boolean | Onemogući čitanje sa `.claude` (prompt + vještine) |
...
| `NANOGPT_SERVER_PASSWORD` | string | Omogući osnovnu autorizaciju za `serve`/`web` |
...
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | broj | Zadano vremensko ograničenje za bash komande u ms |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX` | broj | Maksimalni izlazni tokeni za LLM odgovore |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER` | boolean | Omogući praćenje datoteka za cijeli dir |
...
| `NANOGPT_EXPERIMENTAL_PLAN_MODE` | boolean | Omogući režim plana |
