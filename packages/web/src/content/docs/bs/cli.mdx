...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenCode-a bez TUI interfejsa. Postavite `NANOGPT_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenCode-u preko web interfejsa. Postavite `NANOGPT_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
| Varijabla                             | Tip     | Opis                                                               |
| ------------------------------------- | ------- | ------------------------------------------------------------------ |
| `NANOGPT_AUTO_SHARE`                 | boolean | Automatski dijeli sesije                                           |
| `NANOGPT_GIT_BASH_PATH`              | string  | Putanja do Git Bash izvršne datoteke na Windows-u                  |
| `NANOGPT_CONFIG`                     | string  | Putanja do konfiguracijskog fajla                                  |
| `NANOGPT_CONFIG_DIR`                 | string  | Putanja do konfiguracijskog direktorija                            |
| `NANOGPT_CONFIG_CONTENT`             | string  | Inline json konfiguracijski sadržaj                                |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolean | Onemogući automatske provjere ažuriranja                           |
| `NANOGPT_DISABLE_PRUNE`              | boolean | Onemogući brisanje (pruning) starih podataka                       |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolean | Onemogući automatsko ažuriranje naslova terminala                  |
| `NANOGPT_PERMISSION`                 | string  | Inline json konfiguracija dozvola                                  |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolean | Onemogući podrazumijevane dodatke (plugins)                        |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolean | Onemogući automatsko preuzimanje LSP servera                       |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Omogući eksperimentalne modele                                     |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | boolean | Onemogući automatsko sažimanje konteksta                           |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boolean | Onemogući čitanje iz `.claude` (prompt + vještine)                 |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Onemogući čitanje `~/.claude/CLAUDE.md`                            |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Onemogući učitavanje `.claude/skills`                              |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolean | Onemogući dohvaćanje modela iz udaljenih izvora                    |
| `NANOGPT_FAKE_VCS`                   | string  | Lažni VCS provajder za potrebe testiranja                          |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolean | Onemogući provjeru vremena datoteke radi optimizacije              |
| `NANOGPT_CLIENT`                     | string  | Identifikator klijenta (zadano na `cli`)                           |
| `NANOGPT_ENABLE_EXA`                 | boolean | Omogući Exa alate za web pretraživanje                             |
| `NANOGPT_SERVER_PASSWORD`            | string  | Omogući osnovnu autentifikaciju za `serve`/`web`                   |
| `NANOGPT_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `opencode`) |
| `NANOGPT_MODELS_URL`                 | string  | Prilagođeni URL za dohvaćanje konfiguracije modela                 |
...
| Varijabla                                       | Tip     | Opis                                              |
| ----------------------------------------------- | ------- | ------------------------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | boolean | Omogući sve eksperimentalne funkcije              |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Omogući otkrivanje ikona                          |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Onemogući kopiranje pri odabiru u TUI             |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Zadano vremensko ograničenje za bash naredbe u ms |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Maksimalni izlazni tokeni za LLM odgovore         |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolean | Omogući praćenje datoteka za cijeli direktorij    |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolean | Omogući oxfmt formatter                           |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolean | Omogući eksperimentalni LSP alat                  |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Onemogući praćenje datoteka                       |
| `NANOGPT_EXPERIMENTAL_EXA`                     | boolean | Omogući eksperimentalne Exa funkcije              |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | boolean | Omogući eksperimentalnu provjeru tipa LSP         |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | boolean | Omogući eksperimentalne Markdown funkcije         |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolean | Omogući Plan mod                                  |
