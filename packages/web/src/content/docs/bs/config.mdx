...
3. **Prilagođena konfiguracija** (`NANOGPT_CONFIG` env var) - prilagođena preinačenja
...
6. **Inline konfiguracija** (`NANOGPT_CONFIG_CONTENT` env var) - runtime preinačenja
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `NANOGPT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `NANOGPT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.
