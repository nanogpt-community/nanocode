...
2. ** 사용자 정의 설정** (`NANOGPT_CONFIG` env var) - 사용자 정의 overrides
3. ** 프로젝트 별 설정** (`opencode.json`) - 프로젝트 별 설정
4. **`.opencode` 디렉토리 ** - 에이전트, 명령, 플러그인
5. ** 인라인 설정** (`NANOGPT_CONFIG_CONTENT` env var) - 런타임 오버라이드
...
`NANOGPT_CONFIG` 환경 변수를 사용하여 사용자 정의 구성 파일 경로 지정.
...
`NANOGPT_CONFIG_DIR`를 사용하여 사용자 정의 구성 디렉토리 지정
