...
3. **Config personalizzata** (variabile d'ambiente `NANOGPT_CONFIG`) - sovrascritture personalizzate
...
6. **Config inline** (variabile d'ambiente `NANOGPT_CONFIG_CONTENT`) - sovrascritture a runtime
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `NANOGPT_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `NANOGPT_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
