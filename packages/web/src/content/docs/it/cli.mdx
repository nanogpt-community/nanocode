...
Avvia un server HTTP che espone accesso API alle funzionalita' di opencode senza la TUI. Imposta `NANOGPT_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `NANOGPT_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `NANOGPT_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `NANOGPT_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `NANOGPT_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `NANOGPT_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `NANOGPT_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `NANOGPT_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `NANOGPT_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolean | Disabilita controllo file time per ottimizzazione           |
| `NANOGPT_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `NANOGPT_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `NANOGPT_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `NANOGPT_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `NANOGPT_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                 |
| ----------------------------------------------- | ------- | ------------------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | boolean | Abilita tutte le funzionalita' sperimentali |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                      |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI         |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms   |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM    |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir       |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                     |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale          |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                     |
| `NANOGPT_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalita' Exa sperimentali      |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita type checking LSP sperimentale      |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | boolean | Abilita markdown sperimentale               |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                           |
