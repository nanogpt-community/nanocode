...
3. **Konfiguracja niestandardowa** (zmienna `NANOGPT_CONFIG`) — jawne nadpisanie
...
6. **Konfiguracja wbudowana** (zmienna `NANOGPT_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `NANOGPT_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `NANOGPT_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.opencode` i powinien zachować tę samą strukturę.
