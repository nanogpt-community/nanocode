---
title: Providers
description: Using any LLM provider in NanoCode.
---

import config from "../../../config.mjs"
export const console = config.console

NanoCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your NanoCode config.
...
in `~/.local/share/nanocode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your NanoCode
config.
...
NanoCode Zen is a list of models provided by the NanoCode team that have been
tested and verified to work well with NanoCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with NanoCode Zen.
:::

1. Run the `/connect` command in the TUI, select nanocode, and head to [nanocode.ai/auth](https://nanocode.ai/auth).
...
It works like any other provider in NanoCode and is completely optional to use.
...
To use Amazon Bedrock with NanoCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running nanocode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY nanocode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile nanocode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX nanocode
   ```
   ...
   For project-specific or persistent configuration, use `nanocode.json`:

   ```json title="nanocode.json"
   {
     "$schema": "https://nanocode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in NanoCode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for nanocode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX nanocode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX nanocode
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your nanocode config.
...
to store token in nanocode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
NanoCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock NanoCode
to only use your own GitLab-hosted instance, add the following to your
`nanocode.json` file. It is also recommended to disable session sharing.
...
More documentation on [nanocode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nanocode-gitlab-auth) homepage.

##### Configuration

Customize through `nanocode.json`:
...
{
  "$schema": "https://nanocode.ai/config.json",
  "plugin": ["@gitlab/nanocode-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with nanocode:
...
To use Google Vertex AI with NanoCode:
...
2. Set the required environment variables:
   ...
   Set them while running nanocode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id nanocode
   ```
...
:::tip
The `global` region improves availability and reduces errors at no extra cost. Use regional endpoints (e.g., `us-central1`) for data residency requirements. [Learn more](https://cloud.google.com/vertex/generative/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
...
In the event you see a feature or model from Helicone that isn't configured automatically through nanocode, you can always configure it yourself.
...
```jsonc title="~/.config/nanocode/nanocode.jsonc"
{
  "$schema": "https://nanocode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [nanocode-helicone-session](https://github.com/H2Shami/nanocode-helicone-session) plugin to automatically log each NanoCode conversation as a session in Helicone.

```bash
npm install -g nanocode-helicone-session
```
...
```json title="nanocode.json"
{
  "plugin": ["nanocode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each NanoCode conversation listed as a separate session.
...
You can configure nanocode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="nanocode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://nanocode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure nanocode to use local models through LM Studio.

```json title="nanocode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://nanocode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure nanocode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for NanoCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/nanocode) for details.
:::

```json title="nanocode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://nanocode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with NanoCode:
...
3. Copy the API key for use in NanoCode.
...
6. **Important**: Before using cloud models in NanoCode, you must pull the model information locally:
...
NanoCode Zen is a list of tested and verified models provided by the NanoCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>NanoCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **NanoCode Zen**.

   ```txt
   /connect
   ```

3. Enter your NanoCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your nanocode config.
...
5. You can also customize them through your nanocode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' nanocode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group nanocode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with NanoCode:
...
You can also customize models through your nanocode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with nanocode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in nanocode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `nanocode.json` file in your project directory:

   ```json title="nanocode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://nanocode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="nanocode.json" {9,11,17-20}
{
  "$schema": "https://nanocode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow NanoCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `nanocode auth list` to see if the credentials
...
2. For custom providers, check the nanocode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your nanocode config.
