...
:::note
Bu arac yalnizca `NANOGPT_EXPERIMENTAL_LSP_TOOL=true` (veya `NANOGPT_EXPERIMENTAL=true`) oldugunda kullanilabilir.
:::
...
:::note
Bu arac yalnizca opencode provider kullanilirken veya `NANOGPT_ENABLE_EXA` ortam degiskeni dogru degerlerden birine ayarlandiginda (ornegin `true` ya da `1`) kullanilabilir.
