...
:::caution
`NANOGPT_SERVER_PASSWORD` ayarlanmadıysa sunucunun güvenliği kaldırılacaktır. Bu, yerel kullanım için iyidir ancak ağ erişimi için ayarlanmalıdır.
:::
...
Erişimi korumak için `NANOGPT_SERVER_PASSWORD` ortam değişkenini kullanarak bir parola ayarlayın:

```bash
NANOGPT_SERVER_PASSWORD=secret opencode web
```

Kullanıcı adı varsayılan olarak `opencode` şeklindedir ancak `NANOGPT_SERVER_USERNAME` ile değiştirilebilir.
