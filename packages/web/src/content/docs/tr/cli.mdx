...
| ------------------------------------- | ------- | --------------------------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boole   | Oturumları otomatik olarak paylaş                                           |
| `NANOGPT_GIT_BASH_PATH`              | dize    | Windows'ta yürütülebilir Git Bash'in Yolu                                   |
| `NANOGPT_CONFIG`                     | dize    | Yapılandırma dosyasının yolu                                                |
| `NANOGPT_CONFIG_DIR`                 | dize    | Yapılandırma dizinine giden yol                                             |
...
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boole   | Otomatik terminal başlığı güncellemelerini devre dışı bırakın               |
| `NANOGPT_PERMISSION`                 | string  | Satır içi JSON izin config'i                                                |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boole   | Varsayılan eklentileri devre dışı bırakın                                   |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boole   | Otomatik LSP sunucu indirmelerini devre dışı bırakın                        |
...
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boole   | `.claude`'den okumayı devre dışı bırak (istem + beceriler)                  |
...
| `NANOGPT_DISABLE_MODELS_FETCH`       | boole   | Uzak kaynaklardan model getirmeyi devre dışı bırakın                        |
| `NANOGPT_FAKE_VCS`                   | dize    | Test amaçlı sahte VCS sağlayıcısı                                           |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boole   | Optimizasyon için dosya süresi kontrolünü devre dışı bırakın                |
| `NANOGPT_CLIENT`                     | string  | Client kimliği (varsayılan: `cli`)                                          |
| `NANOGPT_ENABLE_EXA`                 | boolean | Exa web arama araçlarını etkinleştir                                        |
| `NANOGPT_SERVER_PASSWORD`            | dize    | `serve`/`web` için temel kimlik doğrulamayı etkinleştirin                   |
| `NANOGPT_SERVER_USERNAME`            | dize    | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `opencode`) |
| `NANOGPT_MODELS_URL`                 | dize    | Model yapılandırmasını almak için özel URL                                  |
...
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | sayı    | MS cinsinden bash komutları için varsayılan zaman aşımı |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | sayı    | LLM yanıtları için maksimum çıktı belirteçleri          |
