...
3. **Özel yapılandırma** (`NANOGPT_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `opencode.json`) - projeye özgü ayarlar
5. **`.opencode` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`NANOGPT_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
`NANOGPT_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`NANOGPT_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
