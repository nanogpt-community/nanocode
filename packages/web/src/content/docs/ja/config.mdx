...
3. **カスタム設定** (`NANOGPT_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `opencode.json`) - プロジェクト固有の設定
5. **`.opencode` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`NANOGPT_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド
...
`NANOGPT_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`NANOGPT_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
