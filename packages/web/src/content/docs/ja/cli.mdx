...
これにより、TUI インターフェイスを使用せずにopencode機能への API アクセスを提供する HTTP サーバーが起動します。 `NANOGPT_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opencode`)。
...
これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて OpenCode にアクセスします。 `NANOGPT_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opencode`)。
...
| 変数                                  | タイプ   | 説明                                                                |
| ------------------------------------- | -------- | ------------------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | ブール値 | セッションを自動的に共有する                                        |
| `NANOGPT_GIT_BASH_PATH`              | 文字列   | Windows 上で実行可能な Git Bash へのパス                            |
| `NANOGPT_CONFIG`                     | 文字列   | 構成ファイルへのパス                                                |
| `NANOGPT_CONFIG_DIR`                 | 文字列   | config ディレクトリへのパス                                         |
| `NANOGPT_CONFIG_CONTENT`             | 文字列   | インライン JSON 構成コンテンツ                                      |
| `NANOGPT_DISABLE_AUTOUPDATE`         | ブール値 | 自動更新チェックを無効にする                                        |
| `NANOGPT_DISABLE_PRUNE`              | ブール値 | 古いデータのプルーニングを無効にする                                |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | ブール値 | terminal titleの自動更新を無効にする                                |
| `NANOGPT_PERMISSION`                 | 文字列   | インライン化された json 権限設定                                    |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | ブール値 | デフォルトのプラグインを無効にする                                  |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | ブール値 | LSP サーバーの自動ダウンロードを無効にする                          |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | ブール値 | 実験モデルを有効にする                                              |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | ブール値 | 自動コンテキスト圧縮を無効にする                                    |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | ブール値 | `.claude` からの読み取りを無効にする (プロンプト + スキル)          |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | ブール値 | `~/.claude/CLAUDE.md` の読み取りを無効にする                        |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | ブール値 | `.claude/skills` のロードを無効にする                               |
| `NANOGPT_DISABLE_MODELS_FETCH`       | ブール値 | リモート ソースからのモデルの取得を無効にする                       |
| `NANOGPT_FAKE_VCS`                   | 文字列   | テスト目的の偽の VCS プロバイダー                                   |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | ブール値 | 最適化のためにファイル時間チェックを無効にする                      |
| `NANOGPT_CLIENT`                     | 文字列   | クライアント識別子 (デフォルトは `cli`)                             |
| `NANOGPT_ENABLE_EXA`                 | ブール値 | Exa Web 検索ツールを有効にする                                      |
| `NANOGPT_SERVER_PASSWORD`            | 文字列   | `serve`/`web` の基本認証を有効にする                                |
| `NANOGPT_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `opencode`) をオーバーライドします。 |
| `NANOGPT_MODELS_URL`                 | 文字列   | モデル設定を取得するためのカスタム URL                              |
...
| 変数                                            | タイプ   | 説明                                             |
| ----------------------------------------------- | -------- | ------------------------------------------------ |
| `NANOGPT_EXPERIMENTAL`                         | ブール値 | すべての実験的機能を有効にする                   |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | ブール値 | アイコン検出を有効にする                         |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | ブール値 | TUI で選択時のコピーを無効にする                 |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | 番号     | bash コマンドのデフォルトのタイムアウト (ミリ秒) |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | 番号     | LLM 応答の最大Output Tokens                      |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | ブール値 | ディレクトリ全体のファイル監視を有効にする       |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | ブール値 | oxfmt フォーマッタを有効にする                   |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | ブール値 | 実験的な LSP ツールを有効にする                  |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | ブール値 | ファイルウォッチャーを無効にする                 |
| `NANOGPT_EXPERIMENTAL_EXA`                     | ブール値 | 実験的な Exa 機能を有効にする                    |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | ブール値 | 実験的な LSP タイプ チェックを有効にする         |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | ブール値 | 試験的なMarkdown機能を有効にする                 |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | ブール値 | プランモードを有効にする                         |
