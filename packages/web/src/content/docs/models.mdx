...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start NanoCode.
...
Here are several models that work well with NanoCode, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [NanoCode Zen](/docs/zen), you would use `nanocode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="nanogpt.jsonc" {7-12,19-24}
{
  "$schema": "https://nanocode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="nanocode.jsonc" {6-21}
{
  "$schema": "https://nanocode.ai/config.json",
  "provider": {
    "nanocode": {
...
Many models support multiple variants with different configurations. NanoCode ships with built-in default variants for popular providers.

### Built-in variants

NanoCode ships with default variants for many providers:
...
```jsonc title="nanocode.jsonc" {7-18}
{
  "$schema": "https://nanocode.ai/config.json",
  "provider": {
    "openai": {
...
When NanoCode starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the NanoCode config.
