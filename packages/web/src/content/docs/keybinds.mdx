...
NanoCode has a list of keybinds that you can customize through the NanoCode config.
...
NanoCode uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The NanoCode desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `nanocode.json`.
