...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `NANOGPT_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `NANOGPT_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `NANOGPT_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `NANOGPT_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `NANOGPT_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `NANOGPT_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `NANOGPT_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `NANOGPT_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `NANOGPT_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolean | Desabilitar verificação de tempo de arquivo para otimização           |
| `NANOGPT_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `NANOGPT_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `NANOGPT_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `NANOGPT_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencode`) |
| `NANOGPT_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | boolean | Habilitar todos os recursos experimentais                 |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `NANOGPT_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar verificação de tipo LSP experimental            |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | boolean | Habilitar recursos experimentais de markdown              |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
