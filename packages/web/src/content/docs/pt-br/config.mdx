...
3. **Configuração personalizada** (`NANOGPT_CONFIG` var de ambiente) - substituições personalizadas
...
6. **Configuração inline** (`NANOGPT_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `NANOGPT_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `NANOGPT_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.
