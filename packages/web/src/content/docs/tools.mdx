...
Tools allow the LLM to perform actions in your codebase. NanoCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in NanoCode.
...
:::note
This tool is only available when `NANOGPT_EXPERIMENTAL_LSP_TOOL=true` (or `NANOGPT_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the NanoCode provider or when the `NANOGPT_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching NanoCode:

```bash
NANOGPT_ENABLE_EXA=1 nanocode
```

:::

```json title="nanocode.json" {4}
{
  "$schema": "https://nanocode.ai/config.json",
  "permission": {
    "websearch": "allow"
