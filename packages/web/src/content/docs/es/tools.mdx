...
:::note
Esta herramienta solo está disponible cuando `NANOGPT_EXPERIMENTAL_LSP_TOOL=true` (o `NANOGPT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCode o cuando la variable de entorno `NANOGPT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenCode:

```bash
NANOGPT_ENABLE_EXA=1 opencode
```
