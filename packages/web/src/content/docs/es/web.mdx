...
:::caution
Si no se establece `NANOGPT_SERVER_PASSWORD`, el servidor no será seguro. Esto está bien para uso local, pero debe configurarse para acceso a la red.
:::
...
Para proteger el acceso, establezca una contraseña utilizando la variable de entorno `NANOGPT_SERVER_PASSWORD`:

```bash
NANOGPT_SERVER_PASSWORD=secret opencode web
```

El nombre de usuario predeterminado es `opencode` pero se puede cambiar con `NANOGPT_SERVER_USERNAME`.
