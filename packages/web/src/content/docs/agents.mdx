...
There are two types of agents in NanoCode; primary agents and subagents.
...
NanoCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
NanoCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

NanoCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `nanogpt.json` config file:
...
- Global: `~/.config/nanogpt/agents/`
- Per-project: `.nanogpt/agents/`

```markdown title="~/.config/nanogpt/agents/review.md"
---
...
If no temperature is specified, NanoCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global NanoCode config and the project specific config.
...
The model ID in your NanoCode config uses the format `provider/model-id`. For example, if you're using [NanoCode Zen](/docs/zen), you would use `nanocode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="nanogpt.json" {3-6,9-12}
{
  "$schema": "https://nanocode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="nanogpt.json" {3-5,8-10}
{
  "$schema": "https://nanocode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `nanocode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/nanogpt-community/nanocode).
:::
...
```markdown title="~/.config/nanogpt/agents/docs-writer.md"
---
...
```markdown title="~/.config/nanogpt/agents/security-auditor.md"
---
