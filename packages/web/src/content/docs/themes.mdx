...
With NanoCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, NanoCode uses our own `nanocode` theme.
...
NanoCode comes with several built-in themes.
...
- Want NanoCode to match their terminal's appearance
...
NanoCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/nanogpt/themes/*.json` or `$XDG_CONFIG_HOME/nanocode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.nanogpt/themes/*.json`
4. **Current working directory** - Defined in `./.nanogpt/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/nanogpt/themes
vim ~/.config/nanogpt/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .nanogpt/themes
vim .nanogpt/themes/my-theme.json
```
...
{
  "$schema": "https://nanocode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
