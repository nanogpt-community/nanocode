...
To debug issues with NanoCode, start by checking the logs and local data it stores on disk.
...
- **macOS/Linux**: `~/.local/share/nanocode/log/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\nanocode\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `nanocode --log-level DEBUG`.

---

## Storage

NanoCode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/nanocode/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\nanocode`
...
NanoCode Desktop runs a local NanoCode server (the `nanocode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `NanoCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/nanocode/nanocode.jsonc` (or `~/.config/nanocode/nanocode.json`)
- **macOS/Linux** (older installs): `~/.local/share/nanocode/nanocode.jsonc`
...
NanoCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/nanocode/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\nanocode\plugins`
- **Project plugins** (only if you use per-project config)
  - `<your-project>/.nanocode/plugins/`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so NanoCode can rebuild it.

1. Quit NanoCode Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nanocode`
- **Linux**: delete `~/.cache/nanocode` (or run `rm -rf ~/.cache/nanocode`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\nanocode`

3. Restart NanoCode Desktop.

---

### Fix server connection issues

NanoCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `nanocode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `NANOGPT_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `NANOGPT_PORT` (or pick a free port) and restart.
...
On Windows, NanoCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with NanoCode's features.

---

### Notifications not showing

NanoCode Desktop only shows system notifications when:

- notifications are enabled for NanoCode in your OS settings, and
...
1. Quit NanoCode Desktop.
2. Find and delete these files (they live in the NanoCode Desktop app data directory):

- `nanocode.settings.dat` (desktop default server URL)
- `nanocode.global.dat` and `nanocode.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with NanoCode:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/nanogpt-community/nanocode/issues**](https://github.com/nanogpt-community/nanocode/issues)
...
2. **Join our Discord**
   ...
   [**nanocode.ai/discord**](https://nanocode.ai/discord)
...
### NanoCode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `nanocode upgrade`
...
To figure out what models you have access to, run `nanocode models`
...
If you encounter API call errors, this may be due to outdated provider packages. nanocode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/nanocode
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\nanocode`

2. Restart nanocode to reinstall the latest provider packages

This will force nanocode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
nanocode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
