...
description: What's new in NanoCode 1.0.
---

NanoCode 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same nanocode server.
...
version. However some older versions of NanoCode always grab latest.

To upgrade manually, run

```bash
$ nanocode upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ nanocode upgrade 0.15.31
```
