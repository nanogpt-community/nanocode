...
这将启动一个 HTTP 服务器，该服务器提供对 opencode 功能的 API 访问，无需 TUI 界面。设置 `NANOGPT_SERVER_PASSWORD` 以启用 HTTP 基本身份验证（用户名默认为 `opencode`）。
...
这将启动 HTTP 服务器并打开 Web 浏览器以通过 Web 界面访问 opencode。设置 `NANOGPT_SERVER_PASSWORD` 以启用 HTTP 基本身份验证（用户名默认为 `opencode`）。
...
| 变量                                  | 类型    | 描述                                      |
| ------------------------------------- | ------- | ----------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boolean | 自动共享会话                              |
| `NANOGPT_GIT_BASH_PATH`              | string  | Windows 上 Git Bash 可执行文件的路径      |
| `NANOGPT_CONFIG`                     | string  | 配置文件路径                              |
| `NANOGPT_CONFIG_DIR`                 | string  | 配置目录的路径                            |
| `NANOGPT_CONFIG_CONTENT`             | string  | 内联 json 配置内容                        |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolean | 禁用自动更新检查                          |
| `NANOGPT_DISABLE_PRUNE`              | boolean | 禁用数据的修剪                            |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolean | 禁用自动终端标题更新                      |
| `NANOGPT_PERMISSION`                 | string  | 内联 json 权限配置                        |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolean | 禁用默认插件                              |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolean | 禁用自动 LSP 服务器下载                   |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | boolean | 启用实验模型                              |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | boolean | 禁用自动上下文压缩                        |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boolean | 禁用从 `.claude` 读取（提示+技能）        |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | 禁用读取 `~/.claude/CLAUDE.md`            |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | 禁用加载 `.claude/skills`                 |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolean | 禁用从远程源获取模型                      |
| `NANOGPT_FAKE_VCS`                   | string  | 用于测试目的的伪造 VCS                    |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolean | 禁用文件时间检查以进行优化                |
| `NANOGPT_CLIENT`                     | string  | 客户端标识符（默认为 `cli`）              |
| `NANOGPT_ENABLE_EXA`                 | boolean | 启用 Exa 网络搜索工具                     |
| `NANOGPT_SERVER_PASSWORD`            | string  | 为 `serve`/`web` 启用基本身份验证         |
| `NANOGPT_SERVER_USERNAME`            | string  | 覆盖基本身份验证用户名（默认 `opencode`） |
| `NANOGPT_MODELS_URL`                 | string  | 用于获取模型配置的自定义 URL              |
...
| 变量                                            | 类型    | 描述                                |
| ----------------------------------------------- | ------- | ----------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | boolean | 启用所有实验性功能                  |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 启用图标发现                        |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | 在 TUI 中禁用选择时复制             |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 命令的默认超时（以毫秒为单位） |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 响应的最大输出 Token            |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolean | 为整个目录启用文件观察器            |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolean | 启用 oxfmt 格式化程序               |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolean | 启用实验性 LSP 工具                 |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 禁用文件观察器                      |
| `NANOGPT_EXPERIMENTAL_EXA`                     | boolean | 启用实验性 Exa 功能                 |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | boolean | 启用实验性 LSP 类型检查             |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | boolean | 启用实验性 Markdown 功能            |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolean | 启用计划模式                        |
