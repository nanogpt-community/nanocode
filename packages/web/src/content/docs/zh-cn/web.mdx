...
:::caution
如果未设置`NANOGPT_SERVER_PASSWORD`，服务器将不安全。这对于本地使用来说很好，但应该针对网路访问进行设置。
:::
...
要保护访问，请使用 `NANOGPT_SERVER_PASSWORD` 环境变数设置密码：

```bash
NANOGPT_SERVER_PASSWORD=secret opencode web
```

The username defaults to `opencode` but can be changed with `NANOGPT_SERVER_USERNAME`.
