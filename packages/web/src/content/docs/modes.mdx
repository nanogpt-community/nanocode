...
:::caution
Modes are now configured through the `agent` option in the nanocode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in nanocode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the nanocode config.
...
nanocode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.nanocode/plans/*.md` to detail the plan itself
...
Configure modes in your `nanogpt.json` config file:
...
- Global: `~/.config/nanocode/modes/`
- Project: `.nanocode/modes/`

```markdown title="~/.config/nanocode/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global nanocode config and the project specific config.
...
Create mode files in `.nanocode/mode/` for project-specific modes or `~/.config/nanocode/mode/` for global modes:

```markdown title=".nanocode/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/nanocode/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
