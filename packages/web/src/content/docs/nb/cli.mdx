...
Dette starter en HTTP-server som gir API tilgang til OpenCode-funksjonalitet uten TUI-grensesnittet. Sett `NANOGPT_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åpner en nettleser for å få tilgang til OpenCode via et nettgrensesnitt. Sett `NANOGPT_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
| ------------------------------------- | ------ | --------------------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boolsk | Del økter automatisk                                                  |
| `NANOGPT_GIT_BASH_PATH`              | streng | Bane til Git Bash-kjørbar på Windows                                  |
| `NANOGPT_CONFIG`                     | streng | Bane til konfigurasjonsfil                                            |
| `NANOGPT_CONFIG_DIR`                 | streng | Bane til konfigurasjonskatalog                                        |
| `NANOGPT_CONFIG_CONTENT`             | streng | Innebygd json-konfigurasjonsinnhold                                   |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolsk | Deaktiver automatiske oppdateringskontroller                          |
| `NANOGPT_DISABLE_PRUNE`              | boolsk | Deaktiver beskjæring av gamle data                                    |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolsk | Deaktiver automatiske terminaltitteloppdateringer                     |
| `NANOGPT_PERMISSION`                 | streng | Innebygd json-tillatelseskonfigurasjon                                |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolsk | Deaktiver standard plugins                                            |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolsk | Deaktiver automatiske LSP servernedlastinger                          |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | boolsk | Aktiver eksperimentelle modeller                                      |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | boolsk | Deaktiver automatisk kontekstkomprimering                             |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boolsk | Deaktiver lesing fra `.claude` (spørsmål + ferdigheter)               |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | boolsk | Deaktiver lesing `~/.claude/CLAUDE.md`                                |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolsk | Deaktiver innlasting av `.claude/skills`                              |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolsk | Deaktiver henting av modeller fra eksterne kilder                     |
| `NANOGPT_FAKE_VCS`                   | streng | Falsk VCS-leverandør for testformål                                   |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolsk | Deaktiver filtidskontroll for optimalisering                          |
| `NANOGPT_CLIENT`                     | streng | Klientidentifikator (standard til `cli`)                              |
| `NANOGPT_ENABLE_EXA`                 | boolsk | Aktiver Exa-nettsøkeverktøy                                           |
| `NANOGPT_SERVER_PASSWORD`            | streng | Aktiver grunnleggende autentisering for `serve`/`web`                 |
| `NANOGPT_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `opencode`) |
| `NANOGPT_MODELS_URL`                 | streng | Egendefinert URL for henting av modellkonfigurasjon                   |
...
| Variabel                                        | Type   | Beskrivelse                                   |
| ----------------------------------------------- | ------ | --------------------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | boolsk | Aktiver alle eksperimentelle funksjoner       |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolsk | Aktiver ikonoppdagelse                        |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolsk | Deaktiver kopi ved valg i TUI                 |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer | Standard tidsavbrudd for bash-kommandoer i ms |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer | Maks Output Tokens for LLM-svar               |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolsk | Aktiver filovervåker for hele dir             |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolsk | Aktiver oxfmt formatter                       |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolsk | Aktiver eksperimentelt LSP-verktøy            |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolsk | Deaktiver filovervåking                       |
| `NANOGPT_EXPERIMENTAL_EXA`                     | boolsk | Aktiver eksperimentelle Exa-funksjoner        |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | boolsk | Aktiver eksperimentell LSP-typekontroll       |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | boolsk | Aktiver eksperimentelle Markdown-funksjoner   |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolsk | Aktiver planmodus                             |
