...
:::caution
Hvis `NANOGPT_SERVER_PASSWORD` ikke er satt, vil serveren være usikret. Dette er greit for lokal bruk, men bør settes for nettverkstilgang.
:::
...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `NANOGPT_SERVER_PASSWORD`:

```bash
NANOGPT_SERVER_PASSWORD=secret opencode web
```

Brukernavnet er standard til `opencode`, men kan endres med `NANOGPT_SERVER_USERNAME`.
