...
Sett `NANOGPT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er standard til `opencode`, eller sett `NANOGPT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `opencode serve` og `opencode web`.
