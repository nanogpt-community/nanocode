...
:::note
Dette verktøyet er bare tilgjengelig når `NANOGPT_EXPERIMENTAL_LSP_TOOL=true` (eller `NANOGPT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `NANOGPT_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
