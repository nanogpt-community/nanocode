---
title: Server
description: Interact with nanocode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `nanocode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an nanocode client can use.
...
```bash
nanocode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `nanocode.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
nanocode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `NANOGPT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `nanocode`, or set `NANOGPT_SERVER_USERNAME` to override it. This applies to both `nanocode serve` and `nanocode web`.

```bash
NANOGPT_SERVER_PASSWORD=your-password nanocode serve
```
...
When you run `nanocode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the nanocode server to interact with nanocode programmatically.
:::

This architecture lets nanocode support multiple clients and allows you to interact with nanocode programmatically.

You can run `nanocode serve` to start a standalone server. If you have the
nanocode TUI running, `nanocode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the NanoCode [IDE](/docs/ide) plugins.
...
The nanocode server exposes the following APIs.
