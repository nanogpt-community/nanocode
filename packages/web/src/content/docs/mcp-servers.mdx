...
You can add external tools to NanoCode using the _Model Context Protocol_, or MCP. NanoCode supports both local and remote servers.
...
You can define MCP servers in your [NanoCode Config](https://nanocode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="nanogpt.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/nanocode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
NanoCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, NanoCode will:
...
If the server requires authentication, NanoCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `nanocode mcp auth <server-name>`.
...
```bash
nanocode mcp auth my-oauth-server
```
...
```bash
nanocode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, NanoCode will store the tokens securely in `~/.local/share/nanocode/mcp-auth.json`.
...
nanocode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in NanoCode, alongside built-in tools. So you can manage them through the NanoCode config like any other tool.
...
```json title="nanogpt.json" {11, 14-18}
{
  "$schema": "https://nanocode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
nanocode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect NanoCode to your Sentry account.
