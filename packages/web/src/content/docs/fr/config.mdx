...
3. **Configuration personnalisée** (`NANOGPT_CONFIG` env var) - remplacements personnalisés
...
6. **Configuration en ligne** (`NANOGPT_CONFIG_CONTENT` env var) - remplacements d'exécution
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `NANOGPT_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `NANOGPT_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.opencode`, et devrait suivre la même structure.
