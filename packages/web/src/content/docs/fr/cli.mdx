...
Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'OpenCode sans l'interface TUI. Définissez `NANOGPT_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à OpenCode via une interface Web. Définissez `NANOGPT_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | booléen | Partager automatiquement des sessions                                             |
| `NANOGPT_GIT_BASH_PATH`              | chaîne  | Chemin vers l'exécutable Git Bash sur Windows                                     |
| `NANOGPT_CONFIG`                     | chaîne  | Chemin d'accès au fichier de configuration                                        |
| `NANOGPT_CONFIG_DIR`                 | chaîne  | Chemin d'accès au répertoire de configuration                                     |
| `NANOGPT_CONFIG_CONTENT`             | chaîne  | Contenu de configuration JSON en ligne                                            |
| `NANOGPT_DISABLE_AUTOUPDATE`         | booléen | Désactiver les vérifications automatiques des mises à jour                        |
| `NANOGPT_DISABLE_PRUNE`              | booléen | Désactiver le nettoyage des anciennes données                                     |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | booléen | Désactiver les mises à jour automatiques des titres du terminal                   |
| `NANOGPT_PERMISSION`                 | chaîne  | Configuration des autorisations JSON intégrées                                    |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | booléen | Désactiver les plugins par défaut                                                 |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | booléen | Désactiver les téléchargements automatiques du serveur LSP                        |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | booléen | Activer les modèles expérimentaux                                                 |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | booléen | Désactiver le compactage automatique du contexte                                  |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | booléen | Désactiver la lecture de `.claude` (prompt + compétences)                         |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | booléen | Désactiver la lecture `~/.claude/CLAUDE.md`                                       |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | booléen | Désactiver le chargement de `.claude/skills`                                      |
| `NANOGPT_DISABLE_MODELS_FETCH`       | booléen | Désactiver la récupération de modèles à partir de sources distantes               |
| `NANOGPT_FAKE_VCS`                   | chaîne  | Faux fournisseur VCS à des fins de test                                           |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | booléen | Désactiver la vérification de l'heure des fichiers pour l'optimisation            |
| `NANOGPT_CLIENT`                     | chaîne  | Identifiant du client (par défaut `cli`)                                          |
| `NANOGPT_ENABLE_EXA`                 | booléen | Activer les outils de recherche Web Exa                                           |
| `NANOGPT_SERVER_PASSWORD`            | chaîne  | Activer l'authentification de base pour `serve`/`web`                             |
| `NANOGPT_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `opencode`) |
| `NANOGPT_MODELS_URL`                 | chaîne  | URL personnalisé pour récupérer la configuration des modèles                      |
...
| Variables                                       | Type    | Description                                                     |
| ----------------------------------------------- | ------- | --------------------------------------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | booléen | Activer toutes les fonctionnalités expérimentales               |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | booléen | Activer la découverte d'icônes                                  |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booléen | Désactiver la copie lors de la sélection dans TUI               |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | numéro  | Délai d'expiration par défaut pour les commandes bash en ms     |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | numéro  | Nombre maximum de jetons de sortie pour les réponses LLM        |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | booléen | Activer l'observateur de fichiers pour l'ensemble du répertoire |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | booléen | Activer le formateur oxfmt                                      |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | booléen | Activer l'outil expérimental LSP                                |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booléen | Désactiver l'observateur de fichiers                            |
| `NANOGPT_EXPERIMENTAL_EXA`                     | booléen | Activer les fonctionnalités Exa expérimentales                  |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | booléen | Activer la vérification expérimentale de type LSP               |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | booléen | Activer les fonctionnalités Markdown expérimentales             |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | booléen | Activer le mode plan                                            |
