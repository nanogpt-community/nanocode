...
Cela démarre un serveur HTTP qui fournit à API un accès à la fonctionnalité opencode sans l'interface TUI. Définissez `NANOGPT_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
| `NANOGPT_DISABLE_PRUNE`              | booléen | Désactiver l'élagage des anciennes données                                        |
...
| `NANOGPT_DISABLE_CLAUDE_CODE`        | booléen | Désactiver la lecture de `.claude` (invite + compétences)                         |
...
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | numéro  | Délai d'expiration par défaut pour les commandes bash dans ms   |
...
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | booléen | Activer la vérification expérimentale du type LSP               |
