...
يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف opencode دون واجهة TUI. اضبط `NANOGPT_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى OpenCode عبر واجهة ويب. اضبط `NANOGPT_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
| المتغير                               | النوع   | الوصف                                                     |
| ------------------------------------- | ------- | --------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boolean | مشاركة الجلسات تلقائيا                                    |
| `NANOGPT_GIT_BASH_PATH`              | string  | مسار ملف Git Bash التنفيذي على Windows                    |
| `NANOGPT_CONFIG`                     | string  | مسار ملف التهيئة                                          |
| `NANOGPT_CONFIG_DIR`                 | string  | مسار دليل التهيئة                                         |
| `NANOGPT_CONFIG_CONTENT`             | string  | محتوى تهيئة JSON مُضمّن                                   |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolean | تعطيل التحقق التلقائي من التحديثات                        |
| `NANOGPT_DISABLE_PRUNE`              | boolean | تعطيل تنقية البيانات القديمة                              |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolean | تعطيل تحديث عنوان terminal تلقائيا                        |
| `NANOGPT_PERMISSION`                 | string  | تهيئة أذونات JSON مُضمّنة                                 |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolean | تعطيل الإضافات الافتراضية                                 |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolean | تعطيل تنزيل خوادم LSP تلقائيا                             |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | boolean | تفعيل النماذج التجريبية                                   |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | boolean | تعطيل ضغط السياق تلقائيا                                  |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boolean | تعطيل القراءة من `.claude` (الموجه + المهارات)            |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | تعطيل قراءة `~/.claude/CLAUDE.md`                         |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | تعطيل تحميل `.claude/skills`                              |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolean | تعطيل جلب النماذج من مصادر بعيدة                          |
| `NANOGPT_FAKE_VCS`                   | string  | مزود VCS وهمي لأغراض الاختبار                             |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolean | تعطيل التحقق من وقت الملف لتحسين الأداء                   |
| `NANOGPT_CLIENT`                     | string  | معرّف العميل (الافتراضي `cli`)                            |
| `NANOGPT_ENABLE_EXA`                 | boolean | تفعيل أدوات بحث الويب من Exa                              |
| `NANOGPT_SERVER_PASSWORD`            | string  | تفعيل المصادقة الأساسية لخادمي `serve`/`web`              |
| `NANOGPT_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `opencode`) |
| `NANOGPT_MODELS_URL`                 | string  | رابط مخصص لجلب تهيئة النماذج                              |
...
| المتغير                                         | النوع   | الوصف                                       |
| ----------------------------------------------- | ------- | ------------------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | boolean | تفعيل جميع الميزات التجريبية                |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | تفعيل اكتشاف الأيقونات                      |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | تعطيل النسخ عند التحديد في TUI              |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | المهلة الافتراضية لأوامر bash بالميلي ثانية |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | الحد الأقصى لرموز المخرجات لاستجابات LLM    |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolean | تفعيل مراقب الملفات لكامل الدليل            |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolean | تفعيل مُنسّق oxfmt                          |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolean | تفعيل أداة LSP تجريبية                      |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | تعطيل مراقب الملفات                         |
| `NANOGPT_EXPERIMENTAL_EXA`                     | boolean | تفعيل ميزات Exa التجريبية                   |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | boolean | تفعيل تدقيق أنواع LSP تجريبي                |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | boolean | تفعيل ميزات markdown تجريبية                |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolean | تفعيل وضع الخطة                             |
