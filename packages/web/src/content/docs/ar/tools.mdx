...
:::note
هذه الأداة متاحة فقط عند ضبط `NANOGPT_EXPERIMENTAL_LSP_TOOL=true` (أو `NANOGPT_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `NANOGPT_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
