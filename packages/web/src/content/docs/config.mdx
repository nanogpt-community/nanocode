...
:::note
The `.opencode` and `~/.config/nanocode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place plugin files in `.opencode/plugins/` or `~/.config/nanocode/plugins/`. You can also load plugins from npm through the `plugin` option.
