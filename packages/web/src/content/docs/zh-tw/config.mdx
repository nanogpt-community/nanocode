...
3. **自定義設定** (`NANOGPT_CONFIG` env var) - 自定義覆寫
4. **專案設定**（專案中的 `opencode.json`）- 專案特定的設定
5. **`.opencode` 目錄** - 代理、指令、外掛
6. **內聯設定** (`NANOGPT_CONFIG_CONTENT` env var) - 執行時覆寫
...
使用 `NANOGPT_CONFIG` 環境變數指定自定義設定檔路徑。
...
使用 `NANOGPT_CONFIG_DIR` 環境變數指定自定義設定目錄。將在該目錄中搜尋代理、指令、模式和外掛，就像標準 `.opencode` 目錄一樣，並且應該遵循相同的結構。
