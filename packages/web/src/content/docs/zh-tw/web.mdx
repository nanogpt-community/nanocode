...
:::caution
如果未設置`NANOGPT_SERVER_PASSWORD`，服務器將不安全。這對於本地使用來說很好，但應該針對網絡訪問進行設置。
:::
...
要保護訪問，請使用 `NANOGPT_SERVER_PASSWORD` 環境變量設置密碼：

```bash
NANOGPT_SERVER_PASSWORD=secret opencode web
```

用戶名默認為`opencode`，但可以使用`NANOGPT_SERVER_USERNAME` 進行更改。
