...
:::caution
如果未設定 `NANOGPT_SERVER_PASSWORD`，伺服器將不安全。這對於本地使用來說很好，但應該針對網路存取進行設定。
:::
...
要保護存取，請使用 `NANOGPT_SERVER_PASSWORD` 環境變數設定密碼：

```bash
NANOGPT_SERVER_PASSWORD=secret opencode web
```

使用者名稱預設為 `opencode`，但可以使用 `NANOGPT_SERVER_USERNAME` 進行更改。
