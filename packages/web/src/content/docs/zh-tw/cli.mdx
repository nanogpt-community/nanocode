...
這將啟動一個 HTTP 伺服器，該伺服器提供對 opencode 功能的 API 存取，而無需 TUI 介面。設定 `NANOGPT_SERVER_PASSWORD` 以啟用 HTTP 基本身分驗證（使用者名稱預設為 `opencode`）。
...
這將啟動 HTTP 伺服器並打開網頁瀏覽器以透過 Web 介面存取 opencode。設定 `NANOGPT_SERVER_PASSWORD` 以啟用 HTTP 基本身分驗證（使用者名稱預設為 `opencode`）。
...
| 變數                                  | 類型    | 說明                                          |
| ------------------------------------- | ------- | --------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boolean | 自動分享工作階段                              |
| `NANOGPT_GIT_BASH_PATH`              | string  | Windows 上 Git Bash 可執行檔案的路徑          |
| `NANOGPT_CONFIG`                     | string  | 設定檔路徑                                    |
| `NANOGPT_CONFIG_DIR`                 | string  | 設定目錄的路徑                                |
| `NANOGPT_CONFIG_CONTENT`             | string  | 內聯 json 設定內容                            |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolean | 禁用自動更新檢查                              |
| `NANOGPT_DISABLE_PRUNE`              | boolean | 禁用舊數據的修剪                              |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolean | 禁用自動終端機標題更新                        |
| `NANOGPT_PERMISSION`                 | string  | 內聯 json 權限設定                            |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolean | 禁用預設外掛                                  |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolean | 禁用自動 LSP 伺服器下載                       |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | boolean | 啟用實驗模型                                  |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | boolean | 禁用自動上下文壓縮                            |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boolean | 禁止從 `.claude` 讀取（提示+技巧）            |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | 禁用讀取 `~/.claude/CLAUDE.md`                |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | 禁用載入 `.claude/skills`                     |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolean | 禁用從遠端來源獲取模型                        |
| `NANOGPT_FAKE_VCS`                   | string  | 用於測試目的的假 VCS 供應商                   |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolean | 禁用檔案時間檢查以進行最佳化                  |
| `NANOGPT_CLIENT`                     | string  | 客戶端標識符（預設為 `cli`）                  |
| `NANOGPT_ENABLE_EXA`                 | boolean | 啟用 Exa 網路搜尋工具                         |
| `NANOGPT_SERVER_PASSWORD`            | string  | 為 `serve`/`web` 啟用基本身分驗證             |
| `NANOGPT_SERVER_USERNAME`            | string  | 覆蓋基本身分驗證使用者名稱（預設 `opencode`） |
| `NANOGPT_MODELS_URL`                 | string  | 用於獲取模型設定的自定義 URL                  |
...
| 變數                                            | 類型    | 說明                                |
| ----------------------------------------------- | ------- | ----------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | boolean | 啟用所有實驗性功能                  |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 啟用圖示探索                        |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | 在 TUI 中禁用選擇時複製             |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 指令的預設超時（以毫秒為單位） |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 回應的最大輸出 tokens           |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolean | 為整個目錄啟用檔案觀察器            |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolean | 啟用 oxfmt 格式化程式               |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolean | 啟用實驗性 LSP 工具                 |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 禁用檔案觀察器                      |
| `NANOGPT_EXPERIMENTAL_EXA`                     | boolean | 啟用實驗性 Exa 功能                 |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | boolean | 啟用實驗性 LSP 類型檢查             |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | boolean | 啟用實驗性 Markdown 功能            |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolean | 啟用計畫模式                        |
