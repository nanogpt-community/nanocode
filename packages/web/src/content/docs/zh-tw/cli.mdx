...
這將啟動一個 HTTP 服務器，該服務器提供對 opencode 功能的 API 訪問，而無需 TUI 接口。設置`NANOGPT_SERVER_PASSWORD`以啟用HTTP基本身份驗證（用戶名默認為`opencode`）。
...
這將啟動 HTTP 服務器並打開 Web 瀏覽器以通過 Web 介面訪問 opencode。設置`NANOGPT_SERVER_PASSWORD`以啟用HTTP基本身份驗證（用戶名默認為`opencode`）。
...
| 變量                                  | 類型    | 描述                                     |
| ------------------------------------- | ------- | ---------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boolean | 自動共享會話                             |
| `NANOGPT_GIT_BASH_PATH`              | string  | Windows 上 Git Bash 可執行文件的路徑     |
| `NANOGPT_CONFIG`                     | string  | 配置文件路徑                             |
| `NANOGPT_CONFIG_DIR`                 | string  | 配置目錄的路徑                           |
| `NANOGPT_CONFIG_CONTENT`             | string  | 內聯 json 配置內容                       |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolean | 禁用自動更新檢查                         |
| `NANOGPT_DISABLE_PRUNE`              | boolean | 禁用舊數據的修剪                         |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolean | 禁用自動 terminal 標題更新               |
| `NANOGPT_PERMISSION`                 | string  | 內聯 json 權限配置                       |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolean | 禁用默認插件                             |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolean | 禁用自動 LSP 服務器下載                  |
...
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | 禁用加載`.claude/skills`                 |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolean | 禁用從遠程源獲取模型                     |
| `NANOGPT_FAKE_VCS`                   | string  | 用於測試目的的假 VCS 提供商              |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolean | 禁用文件時間檢查以進行優化               |
| `NANOGPT_CLIENT`                     | string  | 客戶端標識符（默認為`cli`）              |
| `NANOGPT_ENABLE_EXA`                 | boolean | 啟用 Exa 網絡搜索工具                    |
| `NANOGPT_SERVER_PASSWORD`            | string  | 為 `serve`/`web` 啟用基本身份驗證        |
| `NANOGPT_SERVER_USERNAME`            | string  | 覆蓋基本身份驗證用戶名（默認`opencode`） |
| `NANOGPT_MODELS_URL`                 | string  | 用於獲取模型配置的自定義 URL             |
...
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 啟用圖標發現                        |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | 在 TUI 中禁用選擇時復制             |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 命令的默認超時（以毫秒為單位） |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 響應的最大輸出令牌              |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolean | 為整個目錄啟用文件觀察器            |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolean | 啟用 oxfmt 格式化程序               |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolean | 啟用實驗性 LSP 工具                 |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 禁用文件觀察器                      |
...
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolean | 啟用計劃模式                        |
