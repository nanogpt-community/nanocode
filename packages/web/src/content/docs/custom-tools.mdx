...
- Locally by placing them in the `.nanogpt/tool/` directory of your project.
- Or globally, by placing them in `~/.config/nanogpt/tool/`.
...
```ts title=".nanogpt/tool/database.ts" {1}
...
```ts title=".nanogpt/tool/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".nanogpt/tool/add.py"
import sys
...
```ts title=".nanogpt/tool/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .nanogpt/tool/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
