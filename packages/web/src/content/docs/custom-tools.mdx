...
- Locally by placing them in the `.nanogpt/tools/` directory of your project.
...
```python title=".nanogpt/tools/add.py"
import sys
...
```ts title=".nanogpt/tools/python-add.ts" {10}
import { tool } from "@nanogpt/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .nanocode/tools/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
