---
title: Custom Tools
description: Create tools the LLM can call in NanoCode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside NanoCode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.nanocode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/nanogpt/tools/`.
...
```ts title=".nanocode/tools/database.ts" {1}
...
```ts title=".nanocode/tools/math.ts"
import { tool } from "@nanogpt/plugin"
...
```ts title=".nanocode/tool/project.ts" {8}
import { tool } from "@nanogpt/plugin"
...
```python title=".nanocode/tools/add.py"
import sys
...
```ts title=".nanocode/tools/python-add.ts" {10}
import { tool } from "@nanogpt/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.directory, ".nanocode/tools/add.py")
