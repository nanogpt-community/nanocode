...
```json title="acp.json"
{
  "agent_servers": {
    "OpenCode": {
      "command": "@nanogpt/absolute/path/bin/opencode",
      "args": ["acp"]
    }
  }
}
...
หากคุณต้องการส่งผ่าน environment variables (เช่น `NANOGPT_API_KEY`) คุณต้องระบุใน `env`:
...
หากคุณต้องส่ง environment variables (เช่น `NANOGPT_API_KEY`) โปรดดู [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) ในเอกสารของ CodeCompanion.nvim สำหรับรายละเอียดทั้งหมด
