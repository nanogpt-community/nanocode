...
ตัวควบคุมจะเริ่มต้นเซิร์ฟเวอร์ HTTP ให้ API เข้าถึงฟังก์ชันการทำงานของ opencode ได้อย่างมีประสิทธิภาพ TUI จัดเตรียม `NANOGPT_SERVER_PASSWORD` ประสิทธิภาพของฮาร์ดแวร์ฮาร์ดแวร์ความถูกต้องพื้นฐาน HTTP (ตัวควบคุมของฮาร์ดแวร์คือ `opencode`)
...
ตัวควบคุมจะเริ่มต้นเซิร์ฟเวอร์ HTTP และเปิดเว็บเองเพื่อเข้าถึง OpenCode ผ่านทางเว็บอินเตอร์เฟส จัดเตรียม `NANOGPT_SERVER_PASSWORD` สำหรับคำอธิบายคุณสมบัติความถูกต้องพื้นฐาน HTTP (ตัวยึดของฮาร์ดแวร์คือ `opencode`)
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | Boolean | แชร์การแจ้งเตือน                                            |
| `NANOGPT_GIT_BASH_PATH`              | String  | เข้าสู่ Git Bash ได้บน Windows                              |
| `NANOGPT_CONFIG`                     | String  | ไปยังไฟล์ฟิลม์                                              |
| `NANOGPT_CONFIG_DIR`                 | String  | เพื่อเป็นไดเร็กทอรีคุณสามารถดู                              |
| `NANOGPT_CONFIG_CONTENT`             | String  | เนื้อหาเหมือนกับ json แบบอินไลน์                            |
| `NANOGPT_DISABLE_AUTOUPDATE`         | Boolean | ตรวจสอบระบบควบคุมอัตโนมัติ                                  |
| `NANOGPT_DISABLE_PRUNE`              | Boolean | การควบคุมการกำจัดข้อมูลเก่า                                 |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | Boolean | เพื่อเป็นชื่อระบบควบคุมอัตโนมัติ                            |
| `NANOGPT_PERMISSION`                 | String  | มีสิทธิ์ json แบบอินไลน์                                    |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | Boolean | การติดตามผลเริ่มต้น                                         |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | Boolean | ค่ำคืนนี้อาหารค่ำ LSP อัตโนมัติ                             |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | Boolean | รูปแบบการทดลอง                                              |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | Boolean | เพื่อการกระชับระบบอัตโนมัติ                                 |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | Boolean | การควบคุมดูแลจาก `.claude` (พร้อมท์ + ทักษะ)                |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | Boolean | การควบคุมดูแล `~/.claude/CLAUDE.md`                         |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | Boolean | ต่อมา `.claude/skills`                                      |
| `NANOGPT_DISABLE_MODELS_FETCH`       | Boolean | ปิดใช้การดึงโมเดลจากแหล่งที่มาของข้อมูล                     |
| `NANOGPT_FAKE_VCS`                   | String  | ประธานาธิบดี VCS ปลอมเพื่อการทดสอบ                          |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | Boolean | ตรวจสอบเวลาไฟล์เพื่อตรวจสอบ                                 |
| `NANOGPT_CLIENT`                     | String  | คนที่ระบุลูกค้า (ส่วนคือ `cli`)                             |
| `NANOGPT_ENABLE_EXA`                 | Boolean | นอกจากนี้เรายังมีเว็บ Exa                                   |
| `NANOGPT_SERVER_PASSWORD`            | String  | เอกสารสิทธิ์พื้นฐานสำหรับ `serve`/`web`                     |
| `NANOGPT_SERVER_USERNAME`            | String  | เมนบอร์ดระบบควบคุมคุณสมบัติความถูกต้องพื้นฐาน ( `opencode`) |
| `NANOGPT_MODELS_URL`                 | String  | ขอขอบคุณ URL สำหรับข้อมูลดึงข้อมูลโมเดล                     |
...
| ตัวแปร                                          | ชนิด    | คำอธิบาย                                       |
| ----------------------------------------------- | ------- | ---------------------------------------------- |
| `NANOGPT_EXPERIMENTAL`                         | Boolean | ในที่สุดเราก็ทดลองทั้งหมด                      |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | Boolean | การแจ้งเตือนการแจ้งเตือน                       |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | Boolean | ยังไงก็ตามการเลือกที่จะเลือกใน TUI             |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | ตัวเลข  | การหมดเวลาเริ่มต้นสำหรับคำสั่ง bash ในหน่วย ms |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | ตัวเลข  | เหตุผลที่สูงสุดสำหรับตอบกลับ LLM               |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | Boolean | เพื่อให้ตัวดูไฟล์สำหรับ dir ทั้งหมด            |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | Boolean | เพื่อให้เป็นไปตามแมตเตอร์ oxfmt                |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | Boolean | สามารถใช้เครื่องมือทดลอง LSP                   |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | Boolean | ไม่ใช่ตัวดูไฟล์                                |
| `NANOGPT_EXPERIMENTAL_EXA`                     | Boolean | คุณสมบัติ Exa ทดลอง                            |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | Boolean | ตรวจสอบประเภท LSP แบบทดลอง                     |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | Boolean | สามารถใช้ Markdown รุ่นทดลองได้                |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | Boolean | เพื่อเป็นโหมด                                  |
