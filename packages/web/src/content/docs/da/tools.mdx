...
:::note
Dette verktøyet er bare tilgængelig når `NANOGPT_EXPERIMENTAL_LSP_TOOL=true` (eller `NANOGPT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `NANOGPT_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
