...
Sett `NANOGPT_SERVER_PASSWORD` for at beskytte serveren med HTTP grunntilføjende autentisering. Brugernavnet er standard til `opencode`, eller sett `NANOGPT_SERVER_USERNAME` for at overstyre det. Dette gjelder både `opencode serve` og `opencode web`.
