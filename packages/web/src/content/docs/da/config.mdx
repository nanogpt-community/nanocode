...
3. **Tilpasset konfiguration** (`NANOGPT_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`opencode.json` i projekt) - projektspecifikke indstillinger
5. **`.opencode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`NANOGPT_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `NANOGPT_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `NANOGPT_CONFIG_DIR`
