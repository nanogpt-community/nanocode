...
Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `NANOGPT_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenCode via en webgrænseflade. Indstil `NANOGPT_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `NANOGPT_AUTO_SHARE`                 | boolean | Del automatisk session                                                |
| `NANOGPT_GIT_BASH_PATH`              | string  | Sti til Git Bash eksekverbar på Windows                               |
| `NANOGPT_CONFIG`                     | string  | Sti til konfigurationsfil                                             |
| `NANOGPT_CONFIG_DIR`                 | string  | Sti til konfigurationsmappe                                           |
| `NANOGPT_CONFIG_CONTENT`             | string  | Indbygget json-konfigurationsindhold                                  |
| `NANOGPT_DISABLE_AUTOUPDATE`         | boolean | Deaktiver automatisk opdateringskontrol                               |
| `NANOGPT_DISABLE_PRUNE`              | boolean | Deaktiver beskæring af gamle data                                     |
| `NANOGPT_DISABLE_TERMINAL_TITLE`     | boolean | Deaktiver automatisk opdatering af terminaltitel                      |
| `NANOGPT_PERMISSION`                 | string  | Indbygget json-tilladelseskonfiguration                               |
| `NANOGPT_DISABLE_DEFAULT_PLUGINS`    | boolean | Deaktiver standard plugins                                            |
| `NANOGPT_DISABLE_LSP_DOWNLOAD`       | boolean | Deaktiver automatisk LSP-serverdownloads                              |
| `NANOGPT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Aktive eksperimentelle modeller                                       |
| `NANOGPT_DISABLE_AUTOCOMPACT`        | boolean | Deaktiver automatisk kontekstkomprimering                             |
| `NANOGPT_DISABLE_CLAUDE_CODE`        | boolean | Deaktiver læsning fra `.claude` (prompt + færdigheder)                |
| `NANOGPT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Deaktiver læsning `~/.claude/CLAUDE.md`                               |
| `NANOGPT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Deaktiver indlæsning af `.claude/skills`                              |
| `NANOGPT_DISABLE_MODELS_FETCH`       | boolean | Deaktivering af modeller fra eksterne kilder                          |
| `NANOGPT_FAKE_VCS`                   | string  | Falsk VCS-udbyder til testformål                                      |
| `NANOGPT_DISABLE_FILETIME_CHECK`     | boolean | Deaktiver filtidskontrol for optimering                               |
| `NANOGPT_CLIENT`                     | string  | Klient-id (standard til `cli`)                                        |
| `NANOGPT_ENABLE_EXA`                 | boolean | Aktiver Exa-websøgeværktøjer                                          |
| `NANOGPT_SERVER_PASSWORD`            | string  | Aktiver grundlæggende godkendelse for `serve`/`web`                   |
| `NANOGPT_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `opencode`) |
| `NANOGPT_MODELS_URL`                 | string  | Brugerdefineret URL til hentning af modelkonfiguration                |
...
| Variabel                                        | Skriv   | Beskrivelse                                |
| ----------------------------------------------- | ------- | ------------------------------------------ |
| `NANOGPT_EXPERIMENTAL`                         | boolean | Aktiver alle eksperimentelle funktioner    |
| `NANOGPT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Aktiver ikonopdagelse                      |
| `NANOGPT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Deaktiver kopi ved valg i TUI              |
| `NANOGPT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer  | Standard timeout for bash-kommandoer i ms  |
| `NANOGPT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer  | Maks. output-tokens for LLM-svar           |
| `NANOGPT_EXPERIMENTAL_FILEWATCHER`             | boolean | Aktiver filovervågning for hele dir        |
| `NANOGPT_EXPERIMENTAL_OXFMT`                   | boolean | Aktiver oxfmt formatter                    |
| `NANOGPT_EXPERIMENTAL_LSP_TOOL`                | boolean | Aktive eksperimenter LSP værktøj           |
| `NANOGPT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Deaktiver filovervågning                   |
| `NANOGPT_EXPERIMENTAL_EXA`                     | boolean | Aktive eksperimenter Exa-funktioner        |
| `NANOGPT_EXPERIMENTAL_LSP_TY`                  | boolean | Aktive forsøg LSP typekontrol              |
| `NANOGPT_EXPERIMENTAL_MARKDOWN`                | boolean | Aktive eksperimentelle markdown-funktioner |
| `NANOGPT_EXPERIMENTAL_PLAN_MODE`               | boolean | Aktiver plantilstand                       |
