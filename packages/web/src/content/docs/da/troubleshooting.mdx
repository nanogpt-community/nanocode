...
Hvis du har `NANOGPT_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `NANOGPT_PORT` (eller vælg en ledig port) og start på ny.
