...
const reg = process.env.REGISTRY ?? "ghcr.io/nanogpt-community"
...
const setup = async () => {
  ...
  if (list.includes("nanocode")) {
    await $`docker buildx use nanocode`
    return
  }
  await $`docker buildx create --name nanocode --use`
}
