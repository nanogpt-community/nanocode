      ...
      async fn() {
        {
          ...
          async function installGitHubApp() {
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.nanogpt.local/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock
        ? "https://dev.github.com/nanogpt-community/nanocode"
        : "https://github.com/nanogpt-community/nanocode"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.nanogpt.local"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "@nanogpt/opencode,/oc")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("nanocode-github-action")
        } catch (error) {
