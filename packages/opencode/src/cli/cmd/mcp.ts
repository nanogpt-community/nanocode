...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .nanocode/ subdirectory too)
  const candidates = [path.join(baseDir, "nanocode.json"), path.join(baseDir, "nanocode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".nanocode", "nanocode.json"), path.join(baseDir, ".nanocode", "nanocode.jsonc"))
  }
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., nanocode x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
