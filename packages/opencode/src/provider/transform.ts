...
export namespace ProviderTransform {
  ...
  export function options(input: {
    ...
    if (
      ...
      (input.model.providerID === "nanogpt" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
    ) {
      result["chat_template_args"] = { enable_thinking: true }
    }

    if (input.model.providerID === "nanogpt" && input.model.capabilities.reasoning === true) {
      if (input.providerOptions?.reasoning_effort) {
        result["reasoning_effort"] = input.providerOptions.reasoning_effort
      }
    }
    ...
    if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
      ...
      if (input.model.providerID.startsWith("nanogpt")) {
        result["promptCacheKey"] = input.sessionID
  ...
  export function maxOutputTokens(
    ...
    options?: Record<string, any>,
    modelLimit?: number,
    globalLimit = OUTPUT_TOKEN_MAX,
  ): number {
    if (typeof input !== "string") {
      return Math.min(input.limit.output, OUTPUT_TOKEN_MAX) || OUTPUT_TOKEN_MAX
    }
  ...
  export function error(providerID: string, error: APICallError) {
    ...
    if (providerID.includes("github-copilot") && error.statusCode === 403) {
      return "Please reauthenticate with the copilot provider to ensure your credentials work properly with NanoCode."
    }
