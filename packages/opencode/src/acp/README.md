...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for nanocode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal nanocode sessions
...
- **Terminal Integration**: Full terminal support via nanocode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | nanocode acp
```
...
ACP sessions map cleanly to nanocode's internal session model:
