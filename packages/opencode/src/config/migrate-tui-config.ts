...
const TUI_SCHEMA_URL = "https://nanocode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from nanocode.json files
...
async function configFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.NANOGPT_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("nanocode", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "nanocode")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "nanocode"))
  }
  if (Flag.NANOGPT_CONFIG) files.push(Flag.NANOGPT_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "nanocode"))
