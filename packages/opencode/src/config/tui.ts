...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.NANOGPT_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.NANOGPT_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.NANOGPT_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".nanocode") && dir !== Flag.NANOGPT_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old nanocode.json shape) still get their settings applied.
    const normalized = (() => {
