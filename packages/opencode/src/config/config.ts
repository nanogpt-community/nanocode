...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/nanocode"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "nanocode")
      default:
        return "/etc/nanocode"
    }
  }
  ...
  export function managedConfigDir() {
    return process.env.NANOGPT_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  export const state = Instance.state(async () => {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://github.com/nanogpt-community/nanocode/config.json"
        result = mergeConfigConcatArrays(
          result,
          await load(JSON.stringify(remoteConfig), {
            dir: path.dirname(`${key}/.well-known/nanocode`),
            source: `${key}/.well-known/nanocode`,
          }),
        )
    ...
    if (!Flag.NANOGPT_DISABLE_PROJECT_CONFIG) {
      for (const file of await ConfigPaths.projectFiles("nanocode", Instance.directory, Instance.worktree)) {
    ...
    if (process.env.NANOGPT_CONFIG_CONTENT) {
      result = mergeConfigConcatArrays(
        result,
        await load(process.env.NANOGPT_CONFIG_CONTENT, {
          dir: Instance.directory,
          source: "NANOGPT_CONFIG_CONTENT",
        }),
      )
    ...
    if (Flag.NANOGPT_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.NANOGPT_DISABLE_PRUNE) {
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.nanocode/command/", "/.nanocode/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.nanocode/agent/", "/.nanocode/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://nanocode.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://nanocode.ai/docs/agents"),
      provider: z
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    const normalized = (() => {
      ...
      delete copy.tui
      log.warn("tui keys in nanocode config are deprecated; move them to tui.json", { path: source })
      return copy
    })()
