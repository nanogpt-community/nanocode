...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.NANOGPT_TEST ?? "")'), {
      env: {
        NANOGPT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "NANOGPT_TEST_SHELL"], {
      shell: true,
      env: {
        NANOGPT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("NANOGPT_TEST_SHELL=ok")
  })
