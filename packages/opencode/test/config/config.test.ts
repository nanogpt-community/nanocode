...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["NANOGPT_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      opencode: {
        options: {
          apiKey: "{env:NANOGPT_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["NANOGPT_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["NANOGPT_CONSOLE_TOKEN"]
    }
  }
})
...
test("does not try to install dependencies in read-only NANOGPT_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.NANOGPT_CONFIG_DIR
  process.env.NANOGPT_CONFIG_DIR = tmp.extra
...
test("installs dependencies in writable NANOGPT_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(scopedEntry?.includes("@nanogpt/node_modules/@scope/plugin/")).toBe(true)
    },
  })
})
...
describe("getPluginName", () => {
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@nanogpt/plugin@2.0.0")).toBe("@nanogpt/plugin")
  })
...
describe("NANOGPT_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NANOGPT_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NANOGPT_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NANOGPT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NANOGPT_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["NANOGPT_CONFIG_DIR"]
      process.env["NANOGPT_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NANOGPT_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["NANOGPT_CONFIG_DIR"]
      } else {
        process.env["NANOGPT_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("NANOGPT_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["NANOGPT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NANOGPT_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["NANOGPT_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NANOGPT_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from NANOGPT_CONFIG_DIR, not project
