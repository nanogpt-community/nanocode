...
afterEach(async () => {
  delete process.env.NANOGPT_CONFIG
  delete process.env.NANOGPT_TUI_CONFIG
...
test("project config takes precedence over NANOGPT_TUI_CONFIG (matches NANOGPT_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.NANOGPT_TUI_CONFIG = custom
    },
  })
...
test("NANOGPT_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from NANOGPT_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.NANOGPT_CONFIG = path.join(customDir, "opencode.json")
    },
  })
