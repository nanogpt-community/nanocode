...
import type { Event, EventMessagePartUpdated, ToolStatePending, ToolStateRunning } from "@nanogpt/sdk/v2"
  ...
  test("routes message.part.delta by the event sessionID (no cross-session pollution)", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const cwd = "@nanogpt/tmp/opencode-acp-test"
      ...
      fn: async () => {
        ...
        controller.push(
          toolEvent(sessionId, cwd, {
            ...
            tool: "read",
            status: "running",
            input: { filePath: "@nanogpt/tmp/example.txt" },
          }),
        )
        await new Promise((r) => setTimeout(r, 20))
