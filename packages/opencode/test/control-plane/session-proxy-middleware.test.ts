...
const original = Flag.NANOGPT_EXPERIMENTAL_WORKSPACES
// @ts-expect-error don't do this normally, but it works
Flag.NANOGPT_EXPERIMENTAL_WORKSPACES = true

afterEach(() => {
  // @ts-expect-error don't do this normally, but it works
  Flag.NANOGPT_EXPERIMENTAL_WORKSPACES = original
})
...
describe("control-plane/session-proxy-middleware", () => {
  test("forwards non-GET session requests for workspaces", async () => {
    ...
    ctx.app.post("@nanogpt/session/foo", (c) => c.text("local", 200))
  ...
  //   ctx.app.get("@nanogpt/session/foo", (c) => c.text("local", 200))
  //   const response = await ctx.request("http://workspace.test/session/foo?x=1")
