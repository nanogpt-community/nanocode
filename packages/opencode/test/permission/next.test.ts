...
test("fromConfig - does not expand tilde in middle of path", () => {
  const result = PermissionNext.fromConfig({ external_directory: { "@nanogpt/some/~/path": "allow" } })
  expect(result).toEqual([{ permission: "external_directory", pattern: "@nanogpt/some/~/path", action: "allow" }])
})
...
test("evaluate - exact match at end wins over earlier wildcard", () => {
  const result = PermissionNext.evaluate("bash", "@nanogpt/bin/rm", [
    { permission: "bash", pattern: "*", action: "allow" },
    { permission: "bash", pattern: "@nanogpt/bin/rm", action: "deny" },
  ])
  expect(result.action).toBe("deny")
})
