...
test("ShareNext.request uses legacy share API without active org account", async () => {
  ...
  try {
    ...
    expect(req.api.create).toBe("@nanogpt/api/share")
    expect(req.api.sync("shr_123")).toBe("@nanogpt/api/share/shr_123/sync")
    expect(req.api.remove("shr_123")).toBe("@nanogpt/api/share/shr_123")
    expect(req.api.data("shr_123")).toBe("@nanogpt/api/share/shr_123/data")
...
test("ShareNext.request uses org share API with auth headers when account is active", async () => {
  ...
  try {
    ...
    expect(req.api.create).toBe("@nanogpt/api/shares")
    expect(req.api.sync("shr_123")).toBe("@nanogpt/api/shares/shr_123/sync")
    expect(req.api.remove("shr_123")).toBe("@nanogpt/api/shares/shr_123")
    expect(req.api.data("shr_123")).toBe("@nanogpt/api/shares/shr_123/data")
