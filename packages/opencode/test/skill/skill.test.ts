...
test("discovers skills from .nanocode/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".nanocode", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".nanocode", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
...
`,
  ...
  const home = process.env.NANOGPT_TEST_HOME
  process.env.NANOGPT_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".nanocode", "skill", "dir-skill")
        expect(dirs).toContain(skillDir)
        expect(dirs.length).toBe(1)
      },
    })
  } finally {
    process.env.NANOGPT_TEST_HOME = home
  }
})

test("discovers multiple skills from .nanocode/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".nanocode", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".nanocode", "skill", "skill-two")
      await Bun.write(
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.NANOGPT_TEST_HOME
  process.env.NANOGPT_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.NANOGPT_TEST_HOME = originalHome
  }
})
