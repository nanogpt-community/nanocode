...
test("explore agent asks for external directories and allows Truncate.GLOB", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const explore = await Agent.get("explore")
      expect(explore).toBeDefined()
      expect(PermissionNext.evaluate("external_directory", "@nanogpt/some/other/path", explore!.permission).action).toBe("ask")
...
test("Truncate.GLOB is allowed even when user denies external_directory globally", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("external_directory", "@nanogpt/some/other/path", build!.permission).action).toBe("deny")
    },
  })
})
