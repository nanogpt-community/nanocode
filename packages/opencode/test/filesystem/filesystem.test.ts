...
describe("AppFileSystem", () => {
  describe("isDir", () => {
    ...
    it(
      ...
      Effect.gen(function* () {
        const fs = yield* AppFileSystem.Service
        expect(yield* fs.isDir("@nanogpt/tmp/nonexistent-" + Math.random())).toBe(false)
      }),
    )
  })
  ...
  describe("pure helpers", () => {
    ...
    test("contains checks path containment", () => {
      expect(AppFileSystem.contains("@nanogpt/a/b", "@nanogpt/a/b/c")).toBe(true)
      expect(AppFileSystem.contains("@nanogpt/a/b", "@nanogpt/a/c")).toBe(false)
    })

    test("overlaps detects overlapping paths", () => {
      expect(AppFileSystem.overlaps("@nanogpt/a/b", "@nanogpt/a/b/c")).toBe(true)
      expect(AppFileSystem.overlaps("@nanogpt/a/b/c", "@nanogpt/a/b")).toBe(true)
      expect(AppFileSystem.overlaps("@nanogpt/a", "@nanogpt/b")).toBe(false)
    })
  })
})
